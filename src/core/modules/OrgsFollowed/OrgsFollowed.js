...
export const OrgsFollowed = () => {
    ...
    const getOrgs = async (searchQuery, searchOffset) => {
        const res = await fetchData(`/api/v3/profile/followed/organizations?limit=${LIMIT.current}&offset=${searchQuery ? typeof searchOffset === 'number' ? searchOffset : offsetSearch : offset}${searchQuery ? `&name=${searchQuery}` : ''}`)
    ...
    useEffect(() => {
        if (window.innerWidth < MOBILE_WIDTH.current) {
                ...
                <CompetitionsAside search setSearchQuery={setSearchQuery} getOrgs={getOrgs}/>
                {
                    orgs.length > 0 ? (
                        <div className={styles.orgs}>
                            <div className={styles.orgsTitle}>
                                {t("TheOrganizers")}
                            </div>
                            <div className={styles.orgsList}>
                                {
                                    orgs.map(org => (
                                        <OrgItem data={org} key={org.id} withName/>
                                    ))
                                }
                            </div>
                        </div>
